...
public class JobQueue {
	...
	private void assignJobs() {
		...
		/*
		 * for (int i = 0; i < jobs.length; i++) { int duration = jobs[i]; int
		 * bestWorker = 0; for (int j = 0; j < numWorkers; ++j) { if (nextFreeTime[j] <
		 * nextFreeTime[bestWorker]) bestWorker = j; } assignedWorker[i] = bestWorker;
		 * startTime[i] = nextFreeTime[bestWorker]; nextFreeTime[bestWorker] +=
		 * duration; }
		 */
	}
